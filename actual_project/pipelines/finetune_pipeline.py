...
output_dir = '/home/mmokkenstorm/sync/qwen_models/finetuned_models/math_instruct'
...
Solve problems step by step, explaining each logical move.
End your response with

Final Answer:
\\boxed{<numeric answer here>}
""".strip()
...
raw = raw.remove_columns([c for c in raw["train"].column_names
...
train_val = raw["train"].train_test_split(test_size=0.05,
                                          seed=42,
                                          shuffle=True)
data = DatasetDict({
...
tokenised = data.map(mask_labels,
                     remove_columns=["text"],
                     desc="🔑 Tokenising"
                     )
